...
static lv_obj_t * window_fly_game;
...
void fly_game_start(lv_obj_t *scr)
{
	score=0;
		if(scr!=NULL) {
		parent_window = scr;
		window_fly_game = lv_obj_create(scr); 
		lv_obj_set_size(window_fly_game, lv_pct(100), lv_pct(100));
		lv_obj_center(window_fly_game);
	...
	lv_obj_clear_flag(window_fly_game, LV_OBJ_FLAG_SCROLLABLE);
	lv_obj_set_style_bg_color(window_fly_game,lv_color_hex(0x000020), LV_PART_MAIN);
	lv_obj_clear_flag(window_fly_game, LV_OBJ_FLAG_SCROLLABLE);
	
	
	botton_exit=lv_btn_create(window_fly_game);
  ...
  exit_lable=lv_label_create(botton_exit);
	...
	fly1=lv_img_create(window_fly_game);
	lv_img_set_src(fly1, &fly);
  lv_img_set_angle(fly1, 2700);
   int init_x= lv_obj_get_width(scr)*48/40;
   int init_y= lv_obj_get_height(scr)/2;
	lv_obj_set_pos(fly1,init_x,init_y);
	
	score_lable=lv_label_create(window_fly_game);
...
{	
	...
	if(score>5000&&enemy_boss.alive==0)
	{
		boss_live_value=100;
		enemy_boss.alive=1;
		enemy_boss.obj=lv_img_create(window_fly_game);
...
void zidan_create()
{
	...
	for(i=0;i<zidanshuliang;i++)
	{
     ...
     zidan[i].obj=lv_btn_create(window_fly_game);
...
void timer_enemy_create_cb(lv_timer_t * t)
{
	...
	for(i=0;i<enemyshuliang;i++)
	{
		if(enemy[i].alive==0)
		{
			...
			enemy[i].obj=lv_img_create(window_fly_game);
				...
				if(enemy[j].alive==1)
				{
				  if((zidan[i].x-enemy[j].x<40&&zidan[i].x-enemy[j].x>-20)&&(zidan[i].y-enemy[j].y<20&&zidan[i].y-enemy[j].y>-30))
				    ...
				    enemy[j].alive=0;		
						...
						lv_obj_t * boomx=lv_img_create(window_fly_game);
