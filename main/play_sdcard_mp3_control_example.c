...
void set_player_vol(int vol){
    ...
    audio_hal_set_volume(g_board_handle->audio_hal, vol);
}

void play_music_index_isr(void) {
     char *url=NULL;
    int current_status = atomic_load(&force_music_idx);
    if(current_status==-1) {
        return;
    }
    ...
    sdcard_list_choose(sdcard_list_handle,current_status , &url);
    ...
    atomic_store(&force_music_idx, -1);
}
...
void app_main(void)
{
    ...
    periph_service_set_callback(input_ser, input_key_service_cb, (void *)g_board_handle);
